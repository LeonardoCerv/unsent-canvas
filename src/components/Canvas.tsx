...
export default function Canvas({ notes, onNoteClick, onCanvasClick, loading = false, updateCursor }: CanvasProps) {
    ...
    // Use all notes directly - no virtual scrolling
    ...
    const isClickOnNote = useCallback(() => {
    ...
    const handleMouseMove = useCallback((e: React.MouseEvent) => {
        ...
        setLastPanPoint({ x: e.clientX, y: e.clientY });
    }, [isDragging, lastPanPoint, screenToWorld, updateCursor]);
    ...
    // Keyboard shortcuts for browser zoom and navigation
    useEffect(() => {
            ...
            if ((e.ctrlKey || e.metaKey)) {
                ...
                switch (e.key) {
                    case 'ArrowUp':
                        ...
                        setViewState(prev => ({
                            ...prev,
                            offsetY: prev.offsetY + panAmount / (prev.zoom * prev.browserZoom)
                        }));
                        break;
                    case 'ArrowDown':
                        e.preventDefault();
                    ...
                    case 'ArrowLeft':
                        ...
                        setViewState(prev => ({
                            ...prev,
                            offsetX: prev.offsetX + panAmount / (prev.zoom * prev.browserZoom)
                        }));
                        break;
                    case 'ArrowRight':
                        e.preventDefault();
    ...
    // Initialize canvas size and track changes
    useEffect(() => {
        const resizeCanvas = () => {
            const canvas = canvasRef.current;
    ...
    return (
        <div
            ...
            role="application"
            aria-label="Interactive canvas for viewing and creating notes"
        >
            ...
            <CanvasAnnouncer
                ...
                isLoading={loading}
                currentPosition={mouseWorldPos}
                zoom={viewState.browserZoom}
            />
            ...
            {/* Connection Status */}
            <ConnectionStatus />
            {/* Position indicator and performance metrics */}
            ...
            <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-sm rounded-lg shadow-lg pointer-events-auto z-50 w-64">
                <div className="flex flex-col gap-2 p-3">
                    ...
                    <ul className="text-xs text-gray-600 space-y-1">
                        <li><span className="font-medium">Pan:</span> Drag with mouse, scroll, or arrow keys</li>
                        ...
                        <li><span className="font-medium">Select Note:</span> Click note card or use <kbd>Tab</kbd></li>
                        <li><span className="font-medium">Horizontal Pan:</span> <kbd>Shift</kbd> + <kbd>Scroll</kbd></li>
                        <li><span className="font-medium">Navigate:</span> <kbd>Arrow keys</kbd></li>
                    </ul>
                </div>
            </div>
        </div>
